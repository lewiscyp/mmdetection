...
model = dict(
    ...
    #     type='ResNet'),
    backbone=dict(
    depth=18,  # 修改为 ResNet-18
    frozen_stages=1,
    init_cfg=dict(checkpoint='torchvision://resnet18', type='Pretrained'),  # 修改为 ResNet-18 预训练模型
    norm_cfg=dict(requires_grad=False, type='BN'),
    norm_eval=True,
    num_stages=4,
    out_indices=(0, 1, 2, 3),  # ResNet-18 仍然有 4 个 stage
    style='pytorch',
    type='ResNet'
),
    bbox_head=dict(
...
test_evaluator = dict(
    ann_file='data/zx_split/annotations/instances_val2017.json',
    backend_args=None,
    format_only=False,
    metric='bbox',
    type='CocoMetric')
...
work_dir = './work_dirs/deformable-detr_r50_16xb2-50e_coco_old'
